...
constexpr auto encoding_is_successful(std::string_view binary, std::string_view encoded) -> bool
{
	...
	static_assert(std::ranges::random_access_range<decltype(binary | to_bytes | encode16)>);
	static_assert(std::ranges::sized_range<decltype(binary | to_bytes | encode16)>);

	return std::ranges::equal(binary | to_bytes | encode16, encoded);
}
...
{
	return try_decode16_to_vector(encoded, std::move(proj))
...
template <std::ranges::view V, typename Proj = std::identity>
	requires std::convertible_to<std::indirect_result_t<Proj, std::ranges::iterator_t<V>>, char>
constexpr auto decoding_works_as_expected(V encoded, decode16_error expected_error, Proj proj = {}) -> bool
{
	return try_decode16_to_vector(encoded, std::move(proj))
	    .transform_error([expected_error](decode16_error error) -> bool { return error == expected_error; })
	    .error_or(false);
}
...
constexpr auto test_is_successful(std::string_view binary, std::string_view encoded, Proj proj = {}) -> bool
{
	return encoding_is_successful(binary, encoded) &&
	       ...
	       decoding_works_as_expected(binary | to_bytes | encode16, binary, std::move(proj)) and
	       std::ranges::equal(try_decode16_to_vector(encoded).value_or(std::vector<std::byte>{}) | encode16,
				  encoded);
}
...
void test_encoding_non_forward_range(std::string_view binary, std::string_view encoded)
{
	...
	assert(std::ranges::equal(view(ss) | to_bytes | encode16, encoded));
}
...
auto main() -> int
{
	...
	static_assert(test_is_successful("\x{01}\x{C0}\x{FF}\x{EE}"sv, "01C0FFEE"sv));
	...
	static_assert(decoding_works_as_expected("01C0FZFF\x{00}AEE"sv, decode16_error::illegal_character));
	static_assert(decoding_works_as_expected("01C0FZFF\x{FF}AEE"sv, decode16_error::illegal_character));
	static_assert(decoding_works_as_expected("01C0FF\x{00}AEE"sv, decode16_error::illegal_character));
	static_assert(decoding_works_as_expected("01C0FF\x{FF}AEE"sv, decode16_error::illegal_character));

	test_decoding_non_forward_range("01C0FZFF\x{00}AEE"sv, decode16_error::illegal_character);
	test_decoding_non_forward_range("01C0FZFF\x{FF}AEE"sv, decode16_error::illegal_character);
	test_decoding_non_forward_range("01C0FF\x{00}AEE"sv, decode16_error::illegal_character);
	test_decoding_non_forward_range("01C0FF\x{FF}AEE"sv, decode16_error::illegal_character);

	// Test decoding an odd number of nibbles

	static_assert(decoding_works_as_expected("01C0FFEE5"sv, decode16_error::missing_character));

	test_decoding_non_forward_range("01C0FFEE5"sv, decode16_error::missing_character);
	...
	static_assert(decoding_works_as_expected("01C0FFEE5"sv | transform([](char c) { return std::pair{c, char{}}; }),
						 decode16_error::missing_character, &std::pair<char, char>::first));
